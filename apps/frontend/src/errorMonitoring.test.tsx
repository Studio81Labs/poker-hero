...
describe("browser error monitoring", () => {
  it("stays disabled without a DSN", () => {
    ...
    expect(browserMonitoringOptions({
  ...
  it("removes poker and request data while retaining stack locations", () => {
        ...
        values: [
          {
            ...
            stacktrace: {
              frames: [
                {
                  filename: "https://poker.example.com/assets/index.js?player=name#fragment",
                  abs_path: "https://poker.example.com/assets/index.js?player=name",
                  lineno: 10,
    ...
    expect(sanitized.exception?.values?.[0]?.value).toBe("Exception details redacted");
    expect(sanitized.exception?.values?.[0]).not.toHaveProperty("future_exception_field");
    expect(sanitized.exception?.values?.[0]?.mechanism).not.toHaveProperty("data");
