...
find the corresponding scrubbed container log. Disable automatic session
tracking and client reports in both SDKs, plus browser replay and all performance
tracing, so the adapters emit exception events only. Initialization and delivery
failures are swallowed at the adapter boundary, leaving optional monitoring
