...
`POKER_POSTFLOP_SOLVER_RANGE_MODE=contextual` uses complete supported
open-and-call history to select position-aware postflop ranges. Set it to
`configured` when the deployment's explicit OOP/IP range strings must always
take precedence.
